...
export async function renderNotificationsWorkspace({ workspaceView, coreServiceAdapter }) {
  workspaceView.innerHTML = `
    <header class="workspace-header">
      <div>
        <p class="eyebrow">Phoenix Core</p>
        <h1 class="workspace-title">Notifications</h1>
        <p class="workspace-subtitle">Your permission-aware notification centre.</p>
      </div>
    </header>
    <section class="card panel">
      <div class="table-state">Loading notifications…</div>
    </section>`;
  ...
  try {
    ...
    if (!notifications.length) {
      workspaceView.innerHTML = `
        ...
        <section class="card panel">
          <div class="empty-state"><div><strong>No notifications</strong>You are up to date.</div></div>
        </section>`;
      return;
    }
    ...
    workspaceView.innerHTML = `
      <header class="workspace-header">
        <div>
          <p class="eyebrow">Phoenix Core</p>
          <h1 class="workspace-title">Notifications</h1>
          <p class="workspace-subtitle">${notifications.length} notification${notifications.length === 1 ? "" : "s"} available to you.</p>
        </div>
      </header>
      <section class="card panel">
        <div class="notification-center-list">${items}</div>
      </section>`;
  } catch (error) {
    workspaceView.innerHTML = `
      <header class="workspace-header">
      ...
      <div class="status error"><strong>Notifications unavailable.</strong> ${escapeHtml(error?.message || "The Core notification service could not be reached.")}</div>`;
  }
}
